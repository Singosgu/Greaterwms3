...
本文档说明了如何在 GitHub Actions 工作流中自动化生成 TUF 仓库所需的 updates 目录信息，确保在Windows平台上都能正确生成必要的元数据文件。该流程会自动覆盖现有数据，确保每次构建都使用最新的配置。
...
3. **平台兼容性** - 使用Windows平台的 Python 命令（python）
